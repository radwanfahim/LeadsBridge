...
document.querySelectorAll('.stat-card, .story').forEach(el => observer.observe(el));
...
document.querySelectorAll('.approach__item').forEach(item => {
  item.querySelector('.approach__item-header').addEventListener('click', () => {
    document.querySelectorAll('.approach__item').forEach(i => i.classList.remove('approach__item--active'));
...
document.querySelectorAll('.reality-card').forEach(el => realityObserver.observe(el));

// ===== HOW WE HELP CARDS SCROLL REVEAL =====
const howweObserver = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      entry.target.classList.add('visible');
...
document.querySelectorAll('.howwe-card').forEach(el => howweObserver.observe(el));
...
(function() {
  ...
  function renderCalendar() {
    if (!daysEl) return;
    daysEl.innerHTML = '';
    labelEl.textContent = months[current.getMonth()] + ' ' + current.getFullYear();
